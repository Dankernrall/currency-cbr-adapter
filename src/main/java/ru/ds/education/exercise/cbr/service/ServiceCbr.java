package ru.ds.education.exercise.cbr.service;
...
public class ServiceCbr {
    ...
    public List<CurrencyModel> cbr(LocalDateTime Date) throws DatatypeConfigurationException {
        List<CurrencyModel> listOfCurrency = new ArrayList<>();
        ...
        for (int i = 0; i < chCodeList.getLength(); i++) {
            ...
            for (int j = 0; j < nodeList.getLength(); j++) {
                ...
                if (currencyName != null && currencyValue != 0) {
                    listOfCurrency.add(new CurrencyModel(currencyName, currencyValue));
